...
public class SpringBootProyectoEverisTeacherApplication implements CommandLineRunner {
  ...
  private static final Logger log =
      LoggerFactory.getLogger(SpringBootProyectoEverisTeacherApplication.class);
  ...
  public void run(String... args) throws Exception {
    ...
    Flux.just(oneCourse, twoCourse).flatMap(c -> service.saveDict(c)).doOnNext(c -> {
      log.info("Cantidad de curso : " + c.getNumCourses());
        ...
        Flux.just(new Teacher("Venon", "Ames", "M", new Date(), "Dni", 4456456, oneCourse),
